...
def accuracy_information(source_evaluative_capacity: float, competence_associate: float, competence_opposer: float,
                         content_evaluation_right: float = 0.5, content_evaluation_wrong: float = 0.5):
    ...
    probability_right = (source_evaluative_capacity * competence_associate +
                         (1 - source_evaluative_capacity) * (1 - competence_opposer)) * content_evaluation_right

    # 2. idem for the wrong alternative
    probability_wrong = (source_evaluative_capacity * (1 - competence_associate) +
                         (1 - source_evaluative_capacity) * competence_opposer) * (1 - content_evaluation_wrong)
...
def expected_accuracy(degree_open_mindedness: int = 10, competence_opposer: float = 0.7,
                      competence_associate: float = 0.6, source_evaluative_capacity: float = 0.5,
                      content_evaluative_capacity: float = 0.5, probability_companion_right: float = 'None'):
    ...
    # 2. Open-minded agent
        ...
        if probability_companion_right == 'None':
            probability_companion_right = accuracy_information(source_evaluative_capacity=source_evaluative_capacity,
        ...
        if (degree_open_mindedness % 2) == 0:
            ...
            probability_right = binom.pmf(degree_open_mindedness / 2, degree_open_mindedness,
                                          probability_companion_right) * competence_associate + \
                                binom.sf(degree_open_mindedness / 2, degree_open_mindedness,
        ...
        # 2.b. Case where degree_open_mindedness is odd and there can be ties.
            ...
            probability_tie = binom.pmf((degree_open_mindedness - 1) / 2, degree_open_mindedness,
                                        probability_companion_right) * competence_associate + \
                              binom.pmf((degree_open_mindedness + 1) / 2, degree_open_mindedness,
                                        probability_companion_right) * (1 - competence_associate)
            ...
            probability_right = binom.pmf((degree_open_mindedness + 1) / 2, degree_open_mindedness,
                                          probability_companion_right) * competence_associate + \
                                binom.sf((degree_open_mindedness + 1) / 2, degree_open_mindedness,
                                         probability_companion_right) + probability_tie / 2
    return probability_right


def figure_individual_calculated_accuracy(source_evaluative_capacity: float = 'None', max_degree_open_mindedness: int = 20,
                                          save: bool = False, filename: str = ''):
    ...
    for competence in competence_values:
        ...
        y_values = [expected_accuracy(competence_associate=competence, competence_opposer=competence,
                                      source_evaluative_capacity=source_evaluative_capacity,
                                      degree_open_mindedness=x) - competence for x in xx_values]
        plt.plot(xx_values, y_values, color=color, label=label)

    # 2. Styling and labelling plot
    plt.rcParams.update({'font.family': font_style['family'], 'font.size': font_style['size']})
    plt.title("Epistemic benefits where the source evaluative capacity ($p_{ES}$) is " +
...
def figure_heatmap_source(degree_open_mindedness: int, advantage: float = 0, save: bool = False, filename: str = ''):
    ...
    for x in range(len(index)):
        for y in range(len(columns)):
            c = index[x]
            p = columns[y]
            data[x, y] = expected_accuracy(degree_open_mindedness=degree_open_mindedness,
                                           competence_opposer=c - advantage,
                                           competence_associate=c, source_evaluative_capacity=p) - c
            if data[x, y] < 0:
                mask[x, y] = True
    ...
    heatmap_style = {'annot': True, 'cmap': cmap, 'mask': mask, 'vmin': vmin, 'vmax': vmax, 'linewidths': .1,
                     'linecolor': 'k', 'cbar_kws': {"ticks": cbar_ticks, "shrink": .66}, 'square': True}
    ...
    titletext: str = 'Epistemic benefits where degree of open-mindedness ($n$) is ' + str(degree_open_mindedness)
    ylabeltext: str = ''
    if advantage > 0:
...
def find_tipping_evaluation_content(competence_associate: float, competence_opposer: float,
                                    source_evaluative_capacity: float, degree_open_mindedness: int = 4):
    ...
    probability_right = expected_accuracy(degree_open_mindedness=degree_open_mindedness,
...
def figure_heatmap_tipping_evaluation_content(degree_open_mindedness: int = 4, save: bool = False, filename: str = ''):
    ...
    for x in range(len(index)):
        for y in range(len(columns)):
            c = index[x]
            p = columns[y]
            data[x, y] = find_tipping_evaluation_content(competence_associate=c, competence_opposer=c,
    ...
    # 3. Styling and labelling plot
    fig.set_title('Content evaluation capacity required for epistemic benefits'
                  '\nwhere degree of open-mindedness ($n$) is ' + str(degree_open_mindedness) +
                  '\nin a homogeneous community', **title_style)
    fig.set_xlabel('Source evaluative capacity ($p_{ES}$)', **label_style)
...
def figure_heatmap_content_only(degree_open_mindedness: int = 4, advantage: float = 0,
                                save: bool = False, filename: str = ''):
    ...
    for x in range(len(index)):
        for y in range(len(columns)):
            ...
            p = columns[y]
            data[x, y] = expected_accuracy(degree_open_mindedness=degree_open_mindedness,
                                           competence_associate=c, competence_opposer=c - advantage,
                                           source_evaluative_capacity=0.5,
                                           content_evaluative_capacity=p) - c
            if data[x, y] < 0:
                mask[x, y] = True
            data[x, y] = round(data[x, y], 2)
    df = pd.DataFrame(data, index=index, columns=columns)
    ...
    # 3. Styling and labelling plot
    titletext: str = 'Epistemic benefits of only content evaluation where degree of \nopen-mindedness ($n$) is ' + \
...
def figure_added_accuracy_content(save: bool = False, filename: str = ''):
    ...
    # 0. Initialize variables
    index = [round(0.55 + 0.05 * y, 2) for y in range(6)]  # values for old companion's accuracy
    index.reverse()
    # columns = [0.6, 0.65, 0.70, 0.75, 0.8, 0.85, 0.9]
    columns = [round(0.55 + 0.05 * x, 2) for x in range(6)]  # values for content evaluation
    data = np.zeros((len(index), len(columns)))
    mask = np.zeros_like(data)
    ...
    for x in range(len(index)):
        for y in range(len(columns)):
            ...
            data[x, y] = companion_accuracy * content_evaluation / \
                         (companion_accuracy * content_evaluation + (1 - companion_accuracy) * (1 - content_evaluation)) \
                         - companion_accuracy
            data[x, y] = round(data[x, y], 2)
    df = pd.DataFrame(data, index=index, columns=columns)
    ...
    heatmap_style = {'annot': True, 'cmap': cmap, 'mask': mask, 'vmin': vmin, 'vmax': vmax, 'linewidths': 0.1,
                     'linecolor': 'k', 'cbar_kws': {"ticks": cbar_ticks, "shrink": .66}, 'square': True}
    plt.rc('font', **font_style)
    plt.figure(figsize=figure_size)
    fig = sns.heatmap(df, **heatmap_style)
    ...
    # 3. Styling and labelling plot
    fig.set_title('Added epistemic value of content evaluation ($p_I$ minus $p_C$)', **title_style)
    fig.set_xlabel('Content evaluative capacity ($p_{EC}$)', **label_style)
    fig.set_ylabel('Companion\'s accuracy ($p_C$)\n (without content evaluation)', **label_style)
...
def save_all_figures():
    ...
    figure_heatmap_source(degree_open_mindedness=2, advantage=0, save=True,
                          filename='figures/Figure_heatmap_source_evaluation_n2')
    figure_heatmap_source(degree_open_mindedness=4, advantage=0, save=True,
                          filename='figures/Figure_heatmap_source_evaluation_n4')
    figure_added_accuracy_content(save=True, filename='figures/Figure_added_accuracy_content')
    figure_heatmap_content_only(degree_open_mindedness=4, save=True, filename='figures/Figure_heatmap_content_only_n4')
    figure_heatmap_tipping_evaluation_content(degree_open_mindedness=4, save=True,
                                              filename='figures/Figure_heatmap_tipping_content')
    figure_individual_calculated_accuracy(source_evaluative_capacity=0.7, max_degree_open_mindedness=20, save=True,
                                          filename='figures/Figure_graph_70_zoom')
    figure_individual_calculated_accuracy(source_evaluative_capacity=0.3, max_degree_open_mindedness=50, save=True,
                                          filename='figures/Figure_graph_30')
    figure_individual_calculated_accuracy(source_evaluative_capacity=1.0, max_degree_open_mindedness=50, save=True,
